...
namespace Proxify.Common.Protocol;

/// <summary>
/// Формат туннельного кадра между прокси-сервером и прокси-клиентом.
///
/// ВАЖНО: кадры этого формата никогда не передаются по проводу напрямую —
/// каждая датаграмма сначала шифруется внешним слоем маскировки
/// (см. WireObfuscator: [12]nonce [16]tag [N]AES-GCM(wireKey, кадр целиком),
/// ключи выведены из зарегистрированного ключа клиента). Магия и типы кадров
/// видны только после расшифровки, поэтому на проводе нет статических сигнатур.
///
/// Внутренний формат (big endian):
///
...
///
/// Кадр AUTH (0x08) передаётся в маскирующей оболочке (содержимое не видно снаружи):
///
///   [1] версия = 1
...
///
/// Кадр AUTH_ACK (0x09) также в маскирующей оболочке:
///
