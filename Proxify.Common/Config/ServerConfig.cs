...
public static class ServerConfig
{
    public static bool TryLoad(string path, out List<ClientConfig> clients, out string? error)
    {
        ...
        {
            if (!TryParseEndpointKey(key, out var protocol, out var publicPort, out var gamePort, out var keyError))
            {
            ...
            var isTcp = protocol == TunnelProtocol.Tcp;
    ...
    /// </summary>
    private static bool TryParseEndpointKey(string raw, out TunnelProtocol protocol, out int publicPort, out int gamePort, out string? error)
    {
